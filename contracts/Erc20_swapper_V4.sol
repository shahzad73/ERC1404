...
contract TokenSwapper {
    ...
    function open(uint256 swapNumber, address _executor, address _openingToken, uint256 _tokensToOpen, address _closingToken, uint256 _tokensToClose, uint256 _expiry) external returns(bool) {
        ...
        if( allSwaps[msg.sender][swapNumber].expiry > 0 ) {
            revert("TokenSwapper: swapNumber already used");            
        }

        if( _expiry <= block.timestamp || _expiry > block.timestamp + 365 days ) {
            revert("TokenSwapper: expiry should be in future, but less than / equal to a year");
        }

        if( _executor == address(0) ) {
            revert("TokenSwapper: Executer address cannot be 0");
        } 
        if( _openingToken == address(0) ) {
            revert("TokenSwapper: Opening Token address cannot be 0");
        } 
        if( _closingToken == address(0) ) {
            revert("TokenSwapper: closing token address cannot be 0");
        } 

        if ( _openingToken == _closingToken ) {
            revert("TokenSwapper: OpeningToken and ClosingToken addresses cannot be same");
        }

        if( _tokensToOpen == 0 ) {
            revert ("TokenSwapper: TokensToOpen cannot be 0");
        }
        if( _tokensToClose == 0 ) {
            revert ("TokenSwapper: TokensToClose cannot be 0");
        }
    ...
    function close(address originator, uint256 swapNumber) external returns(bool) {
        ...
        if( msg.sender != swap.executor ) { 
            revert ("TokenSwapper: incorrect swap executor");
        }

        if( block.timestamp > swap.expiry ) { 
            revert ( "TokenSwapper: swap expiration passed" );
        }
    ...
    function expire(address originator, uint256 swapNumber) external returns(bool) {
        ...
        if (block.timestamp <= swap.expiry) {
            revert( "TokenSwapper: swap not yet expired" );
        } 
